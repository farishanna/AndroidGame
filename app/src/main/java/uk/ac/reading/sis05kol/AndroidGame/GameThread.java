...
public abstract class GameThread extends Thread {
	...
	public static final int STATE_LOSE_FINAL = 6;
	...
	public boolean onTouch(MotionEvent e) {
		...
		if(mMode == STATE_READY || mMode == STATE_LOSE || mMode == STATE_WIN || mMode == STATE_LOSE_FINAL) {
			doStart();
			return true;
		}
			...
			if (mMode == STATE_RUNNING) {
				...
				if (mMode == STATE_READY)
					...
					if (mMode == STATE_PAUSE)
						...
						else if (mMode == STATE_LOSE_FINAL){
							mainActivity.lost();
						}
						else 
							if (mMode == STATE_WIN) {
	...
	public void setLives(int lives) {
		...
		synchronized (monitor) {
			...
			b.putString("text", Integer.toString(lives));
			msg.setData(b);
			mHandler.sendMessage(msg);
		}
	}
	...
	protected CharSequence getScoreString() { return Long.toString(Math.round(this.score)); }
